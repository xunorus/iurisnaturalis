async function mint() {
    ...
    Swal.fire("stay in touch..", "this feature is not available yet", "question");
}
        ...
        if (walletChoice == "LOCALWALLET") {
            ...
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let wallet;
                    try {
                        wallet = ethers.Wallet.fromEncryptedJson(encryptedjson, result.value, decryptProgress).then(async (localWallet)=>{
                            // STEP 0
                            w = localWallet;
                            ...
                            let metadataString;
                            try {
                                metadataString = await prepareMetadata();
                            } catch (error) {
                                console.log(error.message);
                                document.getElementById("step1Spinner").style.display = "none";
                                document.getElementById("step1Check").style.visibility = "visible";
                                document.getElementById("step1Check").innerHTML = '<i class="fa-solid fa-circle-xmark" style="color:red"></i>';
                                let printCIDMessage = `<div class="alert alert-warning alert-dismissible fade show" role="alert">${error.message} </div>`;
                            ...
                            try {
                                CID = await client.put(jsonBlobFile, {
                     ...
                     <div id="chainSelector">
                      ...
                      <span id="timer"></span> 
                            ...
                            setTimeout(async ()=>{
                                ...
                                try {
                                    tokenIdbyAddr = await contract.TIds(uAddress);
                                } catch (error) {
                                ...
                                try {
                                    tx = await contractWithSigner.mintUniversalPass(ipfsLink, {
                                        gasLimit: gasLimit
                                    });
                                } catch (error) {
                                    console.log("error:", error);
...
<div id="burnstep0Progress"></div>`;
                let wallet;
                try {
                    wallet = ethers.Wallet.fromEncryptedJson(encryptedjson, result.value, burndecryptProgress).then(async (localWallet)=>{
                        // STEP 0
                        // w=localWallet;
                        console.log("WALLET DECRYPTED: ", localWallet.address);
  ...
  <div id="burnstep1Progress"></div>`;
                        ...
                        let provider;
                        try {
                            provider = await new ethers.providers.JsonRpcProvider(`${optionsList[0].API}`);
                        } catch (error) {
                            console.log("error:", error);
                            return;
                        }
                        ...
                        try {
                            tokenIdbyAddr = await contract.TIds(localWallet.address);
                        } catch (error) {
                            console.log("error:", error);
                            return;
                        }
                        ...
                        try {
                            tx = await contractWithSigner.burnUniversalPass(tId, {
                                gasLimit: gasLimit
                            });
                        } catch (error) {
                            console.log("error:", error);
...
`;
    ...
    getAddressBalance(userAddress).then((balance1)=>{
        console.log(`Balance of ${userAddress}: ${balance1} ETH`);
        document.getElementById("amount").innerHTML = ` ${balance1} ETH`;
    }).catch((error)=>{
...
//# sourceMappingURL=index.0aefd59b.js.map
