...
function writepassword(f) {
    ...
    Swal.fire({
        ...
        inputValidator: (value)=>{
            if (!value) return "You need to set a password to create your wallet!";
        },
        allowOutsideClick: ()=>{
            const popup = Swal.getPopup();
            popup.classList.remove("swal2-show");
...
async function useLocalWallet() {
    ...
    setTimeout(()=>{
        ...
        if (/Mobi|Android/i.test(navigator.userAgent)) console.log("textEffect avoided");
        else {
            $("#usrAddr").textEffect();
            setTimeout(()=>{
                // quick dirty fix
    ...
    // CALL TIds
    try {
        tokenIdbyAddr = await contract.TIds(userAddress);
    } catch (error) {
    ...
    if (tid == 0) console.log("YOU DONT HAVE A UNIVERSAL PASS ");
    else {
        ...
        try {
            universalPassData = await contract.universalPass(tid);
        } catch (error) {
            console.log("error:", error);
            return;
        }
        ...
        if (UPuri == "") {
            ...
            document.getElementById("loader").style.display = "none";
            return;
        } else console.log("URI IS SET", UPuri);
        let cidcid = await fetch(universalPassData.uri).then(function(response) {
        ...
        fetch(cidcid.url).then((response)=>response.text()).then(async (html)=>{
    ...
    if (!ipfsDistros) {
        ...
        // </div>`
            ...
            if (!offchainAttestation) console.log("THERE IS NO  OFFCHAIN ATTESTATION");
            else console.log("YES THERE IS OFFCHAIN ATTESTATION");
...
async function useMetamaskWallet() {
    ...
    // SET ADDRESS
    var shortAddr1 = account.substring(0, 6) + "..." + account.substring(38, 42);
    setTimeout(()=>{
        document.getElementById("usrAddr").innerHTML = `<div id="" onclick="event.stopPropagation();copy2clipboard('${account}')">${shortAddr1}</div>`;
        if (/Mobi|Android/i.test(navigator.userAgent)) console.log("textEffect avoided");
        else {
            $("#usrAddr").textEffect();
            setTimeout(()=>{
                // quick dirty fix
                document.getElementById("usrAddr").innerHTML = `<div id="" onclick="event.stopPropagation();copy2clipboard('${userAddress}')">${shortAddr1}</div>`;
            }, 2000);
        }
    }, 900);
    document.getElementById("usrAddr").classList.add("glow");
...
async function restoreWallet() {
    ...
    seedTextarea.addEventListener("input", function(evt) {
        ...
        if (cntwrds >= 12) setWalletRestore.disabled = false;
        else setWalletRestore.disabled = true;
    });
}
...
async function restoreSeed(seed) {
    ...
    if (!seedPassword.value) {
        ...
        return;
    } else seedPassword.classList.remove("is-invalid");
    // ethers.Wallet.fromMnemonic( mnemonic [ , path , [ wordlist ] ] ) ⇒ Wallet
    // If path is not specified, the Ethereum default path is used (i.e. m/44'/60'/0'/0/0).
    const wallet1 = ethers.Wallet.fromMnemonic(seedTextarea.value.trimStart().trimEnd());
    ...
    const promisseJSON = wallet1.encrypt(seedPassword.value, progressCallback);
    promisseJSON.then((jsonWallet)=>{
        console.log(jsonWallet);
        console.log("WALLET ENCRYPTED");
        // loaderCW
...
function openWalletSend() {
    ...
    numberInput.addEventListener("input", function() {
        ...
        numberInput.value = inputValue.replace(/[^0-9.]/g, "");
    });
...
//# sourceMappingURL=index.08b90290.js.map
