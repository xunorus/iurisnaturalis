...
function plToggle(event) {
    ...
    $(document).click(function(event) {
        if ($container.hasClass("isOpen") && !$(".hamburger")) console.log("NAVLI");
        if ($container.hasClass("isOpen") && !$(event.target).children()) {
            console.log("HAS CLICKEADO FUERA y el menu playlist se cerrar\xe1");
            navBtn.classList.toggle("is-active");
            $(".content").toggleClass("isOpen");
            $(".sidebar").toggleClass("isOpen");
            return false;
...
function handleNavClick(evt) {
    ...
    if (!evt.target.closest(".is-active")) // if (!evt.target.closest(".sidebar")) {
    handleNavlClose();
}
function handleNavlClose(evt) {
    console.log("CLOSE NAVBAR");
}
/** CLOSE MAIN NAVIGATION WHEN CLICKING OUTSIDE THE MAIN NAVIGATION AREA**/ $(document).on("click", function(e) {
    /* bootstrap collapse js adds "in" class to your collapsible element*/ var menu_opened = $(".sidebar").hasClass("is-active");
    if (!$(e.target).closest(".sidebar").length && !$(e.target).is(".sidebar") && menu_opened === true) console.log("YEP. CLOSE NAVBAR");
});
/*********************************************************************************************
  .) DARKMODE
...
toggleButton.addEventListener("click", ()=>{
    ...
    if (document.body.classList.contains("my-class-name")) //do something
...
if (uimode) {
    ...
    document.body.classList.toggle("dark-mode");
} else console.log("no hay uimode");

//# sourceMappingURL=index.f2094c66.js.map
